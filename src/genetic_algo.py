...
def generate_population(Map, cost_map, pop_size, budget):
    population = []
    valid_coords = np.argwhere(Map == 1)
    
    if len(valid_coords) == 0:
        return [np.zeros_like(Map) for _ in range(pop_size)]

    while len(population) < pop_size:
        Map_pop = np.zeros_like(Map)
        sum_cost = 0.0
        shuffled_indices = np.random.permutation(len(valid_coords))
        
        for idx in shuffled_indices:
            j, k = valid_coords[idx]
            c = cost_map[j, k]
            if sum_cost + c <= budget:
                Map_pop[j, k] = 2
                sum_cost += c
            if sum_cost >= budget:
                break
...
def crossover(parent1, parent2, productivity_map, proximity_map, cost_map, budget, generation):
    ...
    sorted_subgroups = [subgroup for _, subgroup in sorted(zip(compactness_total, subgroup_coords_total), reverse=False)]
